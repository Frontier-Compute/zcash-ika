"use strict";
var __defProp = Object.defineProperty;
var __getOwnPropDesc = Object.getOwnPropertyDescriptor;
var __getOwnPropNames = Object.getOwnPropertyNames;
var __hasOwnProp = Object.prototype.hasOwnProperty;
var __export = (target, all) => {
  for (var name in all)
    __defProp(target, name, { get: all[name], enumerable: true });
};
var __copyProps = (to, from, except, desc) => {
  if (from && typeof from === "object" || typeof from === "function") {
    for (let key of __getOwnPropNames(from))
      if (!__hasOwnProp.call(to, key) && key !== except)
        __defProp(to, key, { get: () => from[key], enumerable: !(desc = __getOwnPropDesc(from, key)) || desc.enumerable });
  }
  return to;
};
var __toCommonJS = (mod) => __copyProps(__defProp({}, "__esModule", { value: true }), mod);
var utils_exports = {};
__export(utils_exports, {
  encodeToASCII: () => encodeToASCII,
  fetchAllDynamicFields: () => fetchAllDynamicFields,
  objResToBcs: () => objResToBcs,
  stringToUint8Array: () => stringToUint8Array,
  u64ToBytesBigEndian: () => u64ToBytesBigEndian
});
module.exports = __toCommonJS(utils_exports);
var import_errors = require("./errors.js");
function objResToBcs(resp) {
  if (resp instanceof Error) {
    throw resp;
  }
  if ("object" in resp) {
    resp = resp.object;
  }
  if (!resp.content) {
    throw new import_errors.InvalidObjectError(`Response bcs missing: ${JSON.stringify(resp.type, null, 2)}`);
  }
  return new Uint8Array(resp.content);
}
async function fetchAllDynamicFields(suiClient, parentId) {
  const allFields = [];
  let cursor = null;
  while (true) {
    const response = await suiClient.core.listDynamicFields({
      parentId,
      cursor
    });
    allFields.push(...response.dynamicFields);
    if (response.cursor === cursor) {
      break;
    }
    cursor = response.cursor;
  }
  return allFields;
}
function encodeToASCII(input) {
  const asciiValues = [];
  for (let i = 0; i < input.length; i++) {
    asciiValues.push(input.charCodeAt(i));
  }
  return Uint8Array.from(asciiValues);
}
function u64ToBytesBigEndian(value) {
  const bigIntValue = BigInt(value);
  const buffer = new ArrayBuffer(8);
  const view = new DataView(buffer);
  view.setBigUint64(0, bigIntValue, false);
  return new Uint8Array(buffer);
}
function stringToUint8Array(input) {
  const asciiValues = [];
  for (let i = 0; i < input.length; i++) {
    asciiValues.push(input.charCodeAt(i));
  }
  return Uint8Array.from(asciiValues);
}
//# sourceMappingURL=utils.js.map
