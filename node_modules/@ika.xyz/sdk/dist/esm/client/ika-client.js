...
class IkaClient {
  ...
  async getMultipleDWallets(dwalletIDs) {
    await this.ensureInitialized();
    return this.client.core.getObjects({
  ...
  async getOwnedDWalletCaps(address, cursor, limit) {
    await this.ensureInitialized();
    const response = await this.client.core.listOwnedObjects({
      owner: address,
      ...
      return {
        dWalletCaps: response2.objects.map(
          (obj) => CoordinatorInnerModule.DWalletCap.parse(objResToBcs(obj))
        ),
        cursor: response2.cursor ?? null,
        hasNextPage: response2.hasNextPage ?? false
      };
    });
    return response;
  }
  ...
  async getProtocolPublicParameters(dWallet, curve) {
    ...
    if (dWallet) {
      ...
      selectedCurve = curve !== void 0 ? curve : fromNumberToCurve(0);
    }
    ...
    const protocolPublicParameters = !networkEncryptionKey.reconfigurationOutputID ? await networkDkgPublicOutputToProtocolPublicParameters(
      selectedCurve,
  ...
  async readTableVecAsRawBytes(tableID) {
    try {
      ...
      if (error instanceof InvalidObjectError || error instanceof ObjectNotFoundError || error instanceof NetworkError) {
        throw error;
      }
  ...
  try {
    ...
    if (coordinator instanceof Error || system instanceof Error) {
      throw new NetworkError(
        ...
        coordinator instanceof Error ? coordinator : system
      );
    }
    ...
    throw new NetworkError("Failed to fetch objects", error);
  }
};
...
fetchEncryptionKeysFromNetwork_fn = async function() {
  try {
    ...
    throw new NetworkError("Failed to fetch encryption keys", error);
  }
};
processBatchedObjects_fn = async function(objectIds, processor) {
  ...
  try {
    ...
    throw new NetworkError("Failed to process batched objects", error);
  }
};
/**
